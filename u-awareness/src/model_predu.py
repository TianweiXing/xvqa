...
class MACNetwork(nn.Module):
    def __init__(self, n_vocab, dim, embed_hidden=300,
        ...
        # SWD: adding variance prediction. The variance should be a scalar (all dims in logits share the same variance.)
        self.classifier_var = nn.Sequential(linear(dim * 3, dim),
                                            nn.ELU(),
                                            linear(dim, 1))
    ...
    def forward(self, image, question, question_len, dropout=0.15):
        ...
        out_ans = self.classifier(out)
        out_var = F.softplus(self.classifier_var(out) )

        return out_ans, out_var